...
### Making the token available to Codex
...
Or as a Codex secret:

```bash
Codex secrets set FIGMA_TOKEN figd_your_token_here
```
...
Goal: a designer clones this directory, opens Codex, and asks for screens that snap together using the imported components.
...
This repo ships a Codex skill plus a slash command that automate the prototyping workflow above. Both are scoped to this repo (under `.Codex/`) and only load when Codex operates inside this directory or one of its worktrees.

- **Skill** — [.Codex/skills/create-screen/SKILL.md](.Codex/skills/create-screen/SKILL.md). Auto-triggers when the user asks to create / draft / mock up / design a screen, page, prototype, or wireframe in English or Spanish ("crea una pantalla de…", "diseña un mock-up…", etc.).
- **Slash command** — [.Codex/commands/create-screen.md](.Codex/commands/create-screen.md). Explicit entry point: `/create-screen [optional description]`.
...
- **Worktree mirroring:** the user works in `main`, not in `.Codex/worktrees/*`. Edit files at the project root directly.
